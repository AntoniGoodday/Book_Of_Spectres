...
public class BattlefieldScript : MonoBehaviour
{
    ...
    void InstantiateBattlefield()
    {
        ...
        for (int x = 0; x < xMax; x++)
        {
            for (int y = 0; y < yMax; y++)
            {
                battleTiles.Add(Instantiate(bTile, new Vector3(x*distanceModifier, y*distanceModifier, this.transform.position.z), Quaternion.Euler(0,0,0), gameObject.transform));
                GameObject _tile = battleTiles[_tileID];
                _tile.name = "BattleTile #" + _tileID + ": " + x + "x || " + y + "y";
                _tile.GetComponent<TileClass>().tileID = _tileID;
                _tile.GetComponent<TileClass>().gridLocation = new Vector2Int(x, y);
                TileColour(_tileID);
